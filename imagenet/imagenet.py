...
parser.add_argument('-sp','--stop-picture', default=None, type=int, metavar='SP',
                    help='Max Number of Pictures to learn from during each epoch')                         
...
parser.add_argument('-ptrain', '--print-freq-train', default=500, type=int,
                    metavar='N', help= 'print frequency during training(default:100)')
parser.add_argument('-ptest', '--print-freq-test', default=10, type=int,
                    metavar='N',help= 'print frequency during testing(default:10)')                    
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        if args.arch.startswith('alexnet'):
            print("Applying Dropout Rate '{}'".format(args.drop_rate))
            model = AlexNet(drop_rate=args.drop_rate)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    tb = SummaryWriter(log_dir=os.path.join('runs/imagenet',args.runfolder))
    ...
    for i, (images, target) in enumerate(train_loader):
        if (args.stop_picture == None) or (args.stop_picture>(counter*args.batch_size)):
